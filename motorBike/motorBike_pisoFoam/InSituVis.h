...
const auto ViewPos = kvs::Vec3{ -2, -2, -0.5}; // viewpoint position
const auto ViewLookAt = kvs::Vec3{ -0.5, 0, -0.5 };
const auto ViewDim = kvs::Vec3ui{ 3, 5, 1 }; // viewpoint dimension
...
public:
    InSituVis( const MPI_Comm world = MPI_COMM_WORLD, const int root = 0 ): BaseClass( world, root )
    {
      ...
      this->setOutputFilename("motorBike");
        ...
        this->importBoundaryMesh("./constant/triSurface/motorBike.stl");
        // Time intervals.
