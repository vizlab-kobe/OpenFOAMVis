...
const auto ViewPos = kvs::Vec3{ 7, 0, 2 }; // viewpoint position
const auto ViewLookAt = kvs::Vec3{ 0, 0, 0 };
...
const auto Viewpoint = []() {
    InSituVis::Viewpoint::Location loc(
                                       ViewDir,
                                       ViewPos,
                                       kvs::Vec3{ 0, 0, 1 }, // up vector
                                       ViewLookAt            // focus point
                                        );
    return InSituVis::Viewpoint{ loc };
  }();
...
private:
    kvs::PolygonObject m_rotor_mesh;
    kvs::PolygonObject m_stator_mesh;
...
public:
    InSituVis( const MPI_Comm world = MPI_COMM_WORLD, const int root = 0 ): BaseClass( world, root )
    {
      ...
      this->setOutputFilename("propeller");
        ...
        this->importBoundaryMesh( "./constant/triSurface/shaft.stl" , m_rotor_mesh);
        this->importBoundaryMesh( "./constant/triSurface/shaftRotating.stl", m_rotor_mesh );
        this->importBoundaryMesh( "./constant/triSurface/stirrer.stl", m_rotor_mesh );
        this->importBoundaryMesh( "./constant/triSurface/sparger.stl", m_rotor_mesh );
        this->importBoundaryMesh( "./constant/triSurface/baffles.stl", m_stator_mesh );
    ...
    {
        if ( !BaseClass::screen().scene()->hasObject( "RotorMesh") )
        {
            const bool visible = BaseClass::world().rank() == BaseClass::world().root ();
            auto* object = new kvs::PolygonObject();
            object->shallowCopy( m_rotor_mesh );
            object->setName( "RotorMesh" );
        ...
        if ( !BaseClass::screen().scene()->hasObject( "StatorMesh") )
        {
            const bool visible = BaseClass::world().rank() == BaseClass::world().root ();
            auto* object = new kvs::PolygonObject();
            object->shallowCopy( m_stator_mesh );
            object->setName( "StatorMesh" );
            object->setVisible( visible );
        ...
        auto* rotor = kvs::PolygonObject::DownCast( BaseClass::screen().scene()->object( "RotorMesh" ) );
        auto* stator = kvs::PolygonObject::DownCast( BaseClass::screen().scene()->object( "StatorMesh" ) );
        if ( rotor )
          {
            const double omega = -5.0;          // omega [rad/s]
            const kvs::Vec3 axis( 0, 0, 1 );     // axis

            double angle_deg = ( omega * sim_time.value ) * ( 180.0 / kvs::Math::PI() );

            kvs::Matrix33f R = kvs::Matrix33f::Rotation( axis, angle_deg );

            kvs::Xform xform = rotor->xform();

            kvs::Xform new_xform( xform.translation(), xform.scaling(), R );
            rotor->setXform( new_xform );
          }        

        // ============================================================
        // Calculate and merge bounding box
        // ============================================================
        // prepare variables
        auto local_min = kvs::Vec3( FLT_MAX, FLT_MAX, FLT_MAX );
        auto local_max = kvs::Vec3( -FLT_MAX, -FLT_MAX, -FLT_MAX );
        bool has_geom = false;

        // compare with vectors
        auto min3 = [] ( const kvs::Vec3& a, const kvs::Vec3& b ) {
            return kvs::Vec3( std::min(a.x(), b.x()), std::min(a.y(), b.y()), std::min(a.z(), b.z()) );
        };
        auto max3 = [] ( const kvs::Vec3& a, const kvs::Vec3& b ) {
            return kvs::Vec3( std::max(a.x(), b.x()), std::max(a.y(), b.y()), std::max(a.z(), b.z()) );
        };

        //calculate local range
        if ( rotor ) {
            local_min = min3( local_min, rotor->minExternalCoord() );
            local_max = max3( local_max, rotor->maxExternalCoord() );
            has_geom = true;
        }
        if ( stator ) {
            local_min = min3( local_min, stator->minExternalCoord() );
            local_max = max3( local_max, stator->maxExternalCoord() );
            has_geom = true;
        }
        for ( auto& object : BaseClass::objects() ) {
            auto* vol = Volume::DownCast( object.get() );
            local_min = min3( local_min, vol->minObjectCoord() );
            local_max = max3( local_max, vol->maxObjectCoord() );
            has_geom = true;
        }

        // if no data
        if ( !has_geom ) {
            local_min = kvs::Vec3(0, 0, 0);
            local_max = kvs::Vec3(0, 0, 0);
        }

        // for calculate one nord
        kvs::Real32 minx = local_min.x();
        kvs::Real32 miny = local_min.y();
        kvs::Real32 minz = local_min.z();

        kvs::Real32 maxx = local_max.x();
        kvs::Real32 maxy = local_max.y();
        kvs::Real32 maxz = local_max.z();
        
        // Integrate all nords data
        BaseClass::world().allReduce( minx, minx, MPI_MIN );
        BaseClass::world().allReduce( miny, miny, MPI_MIN );
        BaseClass::world().allReduce( minz, minz, MPI_MIN );
        BaseClass::world().allReduce( maxx, maxx, MPI_MAX );
        BaseClass::world().allReduce( maxy, maxy, MPI_MAX );
        BaseClass::world().allReduce( maxz, maxz, MPI_MAX );

        const kvs::Vec3 global_min( minx, miny, minz );
        const kvs::Vec3 global_max( maxx, maxy, maxz );

        // Adaptation for all objects
        for ( auto& object : BaseClass::objects() )
        {
        ...
        if ( rotor )  { rotor->setMinMaxExternalCoords( global_min, global_max ); }
        if ( stator ) { stator->setMinMaxExternalCoords( global_min, global_max ); }
        // ============================================================
    ...
    void importBoundaryMesh( const std::string& filename, kvs::PolygonObject& target_mesh)
    {
//      m_boundary_mesh = kvs::PolygonImporter( filename );
    
      kvs::PolygonObject new_mesh = kvs::PolygonImporter( filename );
      const auto scale = 1.0f;
      
      auto coords = new_mesh.coords();
      for ( auto& p : coords ) { p *= scale; }
      new_mesh.setCoords( coords );
   
      if( target_mesh.numberOfVertices() == 0)
        {
          target_mesh = new_mesh;
        }
      else
        {
          std::vector<kvs::Real32> v_coords( target_mesh.coords().pointer(), target_mesh.coords().pointer() + target_mesh.coords().size() );
          std::vector<kvs::Real32> v_normals( target_mesh.normals().pointer(), target_mesh.normals().pointer() + target_mesh.normals().size() );
          std::vector<kvs::UInt32> v_connections( target_mesh.connections().pointer(), target_mesh.connections().pointer() + target_mesh.connections().size() );
               
          const kvs::UInt32 offset = target_mesh.numberOfVertices();
   
          std::vector<kvs::Real32> new_v_coords( new_mesh.coords().pointer(), new_mesh.coords().pointer() + new_mesh.coords().size() );
          std::vector<kvs::Real32> new_v_normals( new_mesh.normals().pointer(), new_mesh.normals().pointer() + new_mesh.normals().size() );
          std::vector<kvs::UInt32> new_v_connections( new_mesh.connections().pointer(), new_mesh.connections().pointer() + new_mesh.connections().size() );
   
          v_coords.insert( v_coords.end(), new_v_coords.begin(), new_v_coords.end() );
          v_normals.insert( v_normals.end(), new_v_normals.begin(), new_v_normals.end() );
   
          if ( new_v_connections.size() > 0 )
            {
              for ( auto& id : new_v_connections ) { id += offset; }
              v_connections.insert( v_connections.end(), new_v_connections.begin(), new_v_connections.end() );
            }
   
          target_mesh.setCoords( kvs::ValueArray<kvs::Real32>( v_coords ) );
          target_mesh.setNormals( kvs::ValueArray<kvs::Real32>( v_normals ) );
               
          if ( v_connections.size() > 0 )
            {
              target_mesh.setConnections( kvs::ValueArray<kvs::UInt32>( v_connections ) );
              target_mesh.setPolygonType( kvs::PolygonObject::Triangle ); 
            }
        }
   
      const auto min_coord = target_mesh.minObjectCoord() * scale;
      const auto max_coord = target_mesh.maxObjectCoord() * scale;
      target_mesh.setMinMaxObjectCoords( min_coord, max_coord );
      target_mesh.setMinMaxExternalCoords( min_coord, max_coord );
   
      // Removing x-max, y-min/max, z-min/max planes and setting normals.
      {
        auto v = target_mesh.coords();
        std::vector<kvs::Real32> filtered_coords;
        std::vector<kvs::Real32> filtered_normals;
        for ( size_t i = 0; i < v.size() / 9; i++ )
          {
            ...
            if ( kvs::Math::Equal( max_coord.x(), v0.x() ) && kvs::Math::Equal( max_coord.x(), v1.x() ) && kvs::Math::Equal( max_coord.x(), v2.x() ) ) continue;
            if ( kvs::Math::Equal( min_coord.y(), v0.y() ) && kvs::Math::Equal( min_coord.y(), v1.y() ) && kvs::Math::Equal( min_coord.y(), v2.y() ) ) continue;
            if ( kvs::Math::Equal( max_coord.y(), v0.y() ) && kvs::Math::Equal( max_coord.y(), v1.y() ) && kvs::Math::Equal( max_coord.y(), v2.y() ) ) continue;
            if ( kvs::Math::Equal( min_coord.z(), v0.z() ) && kvs::Math::Equal( min_coord.z(), v1.z() ) && kvs::Math::Equal( min_coord.z(), v2.z() ) ) continue;
            if ( kvs::Math::Equal( max_coord.z(), v0.z() ) && kvs::Math::Equal( max_coord.z(), v1.z() ) && kvs::Math::Equal( max_coord.z(), v2.z() ) ) continue;
   
            filtered_coords.push_back( v0.x() ); filtered_coords.push_back( v0.y() ); filtered_coords.push_back( v0.z() );
            filtered_coords.push_back( v1.x() ); filtered_coords.push_back( v1.y() ); filtered_coords.push_back( v1.z() );
            filtered_coords.push_back( v2.x() ); filtered_coords.push_back( v2.y() ); filtered_coords.push_back( v2.z() );
            filtered_normals.push_back( n0.x() ); filtered_normals.push_back( n0.y() ); filtered_normals.push_back( n0.z() );
          }
        target_mesh.setCoords( kvs::ValueArray<kvs::Real32>( filtered_coords ) );
        target_mesh.setNormals( kvs::ValueArray<kvs::Real32>( filtered_normals ) );
      }
    }   
...
inline InSituVis::Pipeline InSituVis::OrthoSlice()
{
    return [&] ( Screen& screen, const Object& object )
    {
        ...
        auto* stator = kvs::PolygonObject::DownCast( screen.scene()->object( "StatorMesh" ) );
        if ( stator )
        {
...
inline InSituVis::Pipeline InSituVis::Isosurface()
{
    return [&] ( Screen& screen, const Object& object )
    {
        ...
        auto* rotor = kvs::PolygonObject::DownCast( screen.scene()->object( "RotorMesh" ) );
        auto* stator = kvs::PolygonObject::DownCast( screen.scene()->object( "StatorMesh" ) );
//        if (  )
//        {
//            const auto min_coord = mesh->minExternalCoord();
...
//        }
        ...
        const auto min_coord = stator ? stator->minExternalCoord() : rotor->minExternalCoord();
        const auto max_coord = stator ? stator->maxExternalCoord() : rotor->maxExternalCoord();
        volume.setMinMaxObjectCoords( min_coord, max_coord );
        volume.setMinMaxExternalCoords( min_coord, max_coord );
