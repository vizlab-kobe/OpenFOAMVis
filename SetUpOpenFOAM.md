...
```
※注意※
Linuxではアルファベットの大文字と小文字を区別します．`GitHub`と`Github`は別ものとして認識されます．以下のマニュアルでは`GitHub`に統一しています．
...
### gccの確認
- gcc-7.5.0, gcc-10.5.0, gcc-11.4.0, gcc-13.3.0にて動作確認済みです．
- Ubuntu 22.04 LTSにはgcc-11.4.0がプリセットされています．そのまま使用することができます．
- Ubuntu 24.04 LTSにはgcc-13.3.0がプリセットされています．これもそのまま使用することができます．
- 自前環境に独自ビルドしたgccを使用すると，OSMesaを準備する際のllvmのビルドの段階で，システムのgccと競合することがあるようです．できるだけシステムに予めビルドされているgccを使用することをおすすめします．
``` 
$ gcc --version
