...
const auto AnalysisInterval = 50; // l: analysis (visuaization) time interval

const auto VisibleBoundingBox = false;
const auto VisibleBoundaryMesh = true;

// For IN_SITU_VIS__VIEWPOINT__*
const auto ViewPos = kvs::Vec3{4, 2.4, 7}; // viewpoint position
...
public:
    InSituVis( const MPI_Comm world = MPI_COMM_WORLD, const int root = 0 ): BaseClass( world, root )
    {
      ...
      this->setOutputFilename("periodicHill");
        ...
        this->importBoundaryMesh("./periodicHill_hills0.stl");
        // Time intervals.
